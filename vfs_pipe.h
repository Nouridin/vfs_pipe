...
void vfs_register_str(const char * name, char * var_ptr, size_t max_len);
void vfs_init(const char * path);
void vfs_cleanup(const char * path);

#endif // VFS_PIPE_H

#ifdef VFS_PIPE_IMPLM
...
static void vfs_grow_if_needed()
{
    ...
    {
        int new_capacity = (entry_capacity == 0) ? 8 : entry_capacity * 2;
        vfs_entry * tmp = realloc(registry, sizeof(vfs_entry) * new_capacity);
...
void vfs_register_str(const char * name, char * var_ptr, size_t max_len)
{
    ...
    if (registry != NULL) {
        strncpy(registry[entry_count].name, name, 63);
        registry[entry_count].name[63] = '\0';
        registry[entry_count].ptr  = (void*)var_ptr;
        registry[entry_count].type = VFS_STRING;
        registry[entry_count].max_len = max_len;
        entry_count++;
    }
    pthread_mutex_unlock(&vfs_lock);
}
...
void vfs_init(const char * path)
{
    ...
    snprintf(cleanup_cmd, sizeof(cleanup_cmd), "fusermount -u %s > /dev/null 2>&1", path);
    system(cleanup_cmd);
